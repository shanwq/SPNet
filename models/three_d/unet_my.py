...
class ConvBlock(nn.Module):
    def __init__(self, input_channels, output_channels,
                 conv_op=nn.Conv2d, kernel_size=None, stride=None, padding = None,
...
class Unet_Mip(nn.Module):
    def __init__(self, in_channels, classes):
        super(Unet_Mip, self).__init__()
        self.n_channels = in_channels
        self.n_classes =  classes
        
        self.conv_blocks_context_0 = nn.Sequential(
            ConvBlock(input_channels=1, output_channels=32, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=32, output_channels=32, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.conv_blocks_context_1= nn.Sequential(
            ConvBlock(input_channels=32, output_channels=64, kernel_size=(3,3), stride=(2,2), padding = (1,1)),
            ConvBlock(input_channels=64, output_channels=64, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.conv_blocks_context_2= nn.Sequential(
            ConvBlock(input_channels=64, output_channels=128, kernel_size=(3,3), stride=(2,2), padding = (1,1)),
            ConvBlock(input_channels=128, output_channels=128, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.conv_blocks_context_3= nn.Sequential(
            ConvBlock(input_channels=128, output_channels=256, kernel_size=(3,3), stride=(2,2), padding = (1,1)),
            ConvBlock(input_channels=256, output_channels=256, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.conv_blocks_context_4= nn.Sequential(
            ConvBlock(input_channels=256, output_channels=320, kernel_size=(3,3), stride=(2,2), padding = (1,1)),
            ConvBlock(input_channels=320, output_channels=320, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.conv_blocks_context_5= nn.Sequential(
            ConvBlock(input_channels=320, output_channels=320, kernel_size=(3,3), stride=(2,2), padding = (1,1)),
            ConvBlock(input_channels=320, output_channels=320, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )


        self.tu_0 = nn.ConvTranspose2d(in_channels=320, out_channels=320, kernel_size=(2,2), 
                                       stride=(2,2), bias=False)
        self.conv_blocks_localization_0= nn.Sequential(
            ConvBlock(input_channels=640, output_channels=320, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=320, output_channels=320, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        
        self.tu_1 = nn.ConvTranspose2d(in_channels=320, out_channels=256, kernel_size=(2,2), 
                                       stride=(2,2), bias=False)
        self.conv_blocks_localization_1 = nn.Sequential(
            ConvBlock(input_channels=512, output_channels=256, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=256, output_channels=256, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.tu_2 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=(2,2), 
                                       stride=(2,2), bias=False)
        self.conv_blocks_localization_2 = nn.Sequential(
            ConvBlock(input_channels=256, output_channels=128, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=128, output_channels=128, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        
        self.tu_3 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=(2,2), 
                                       stride=(2,2), bias=False)
        self.conv_blocks_localization_3 = nn.Sequential(
            ConvBlock(input_channels=128, output_channels=64, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=64, output_channels=64, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        
        self.tu_4 = nn.ConvTranspose2d(in_channels=64, out_channels=32, kernel_size=(2,2), 
                                       stride=(2,2), bias=False)

        self.conv_blocks_localization_4 = nn.Sequential(
            ConvBlock(input_channels=64, output_channels=32, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
            ConvBlock(input_channels=32, output_channels=32, kernel_size=(3,3), stride=(1,1), padding = (1,1)),
        )
        self.pred_soft = ConvBlock(input_channels=32, output_channels=1, kernel_size=(3,3), stride=(1,1), padding = (1,1))
...
if __name__ == '__main__':
    ...
    Unet_mip = Unet_Mip(1,2)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    Unet_mip = Unet_mip.cuda()
    # TransU3d = trans_unet_3d.cuda()
    input_tensor = torch.randn(1, 1, 192, 192).cuda()
