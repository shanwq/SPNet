...
from hparam_ADAM import hparams as hp
...
def compute_loss(output, target, is_max=True, is_c2f=False, is_sigmoid=True, is_max_hungarian=True, is_max_ds=True, point_rend=False, num_point_rend=None, no_object_weight=None):
    ...
    if isinstance(output, (tuple, list, dict)):
        ...
        # print(' isinstance(output, (tuple, list, dict)', max_ds_loss_weights)

    if is_max and is_max_hungarian:
        # print('is_max and is_max_hungaria')
        # quit()
        ...
        # print('aux_outputs', len(aux_outputs))
        
        # print(aux_outputs[0]['pred_logits'].shape, aux_outputs[0]['pred_masks'].shape)
        # print(aux_outputs[1]['pred_logits'].shape, aux_outputs[1]['pred_masks'].shape)
        # print(aux_outputs[2]['pred_logits'].shape, aux_outputs[2]['pred_masks'].shape)
        ...
        # print('target_sum',target_sum.shape, target_sum)
        targets = []
        for b in range(len(target_onehot)):
            # print(b)
            target_mask = target_onehot[b][target_sum[b] > 0] # (K, D, H, W)
            # print('target_mask.shape', target_onehot[b].shape, target_mask.shape)
            target_label = torch.nonzero(target_sum[b] > 0).squeeze(1) # (K)
            # print('target_label.shape',target_sum[b].shape, target_mask.shape)
        ...
        # print('max_ds_loss_weights[0]', max_ds_loss_weights[0], max_ds_loss_weights)
        loss_list.append(max_ds_loss_weights[0] * loss_final)
        if is_max_ds and "aux_outputs" in output:
            # print('if is_max_ds and "aux_outputs" in output')
            for i, aux_outputs in enumerate(output["aux_outputs"][::-1]): # reverse order
                # print('aux_outputs.shape is', aux_outputs['pred_logits'].shape, aux_outputs['pred_masks'].shape, targets[0]['labels'].shape, targets[0]['masks'].shape)
                # print('aux_outputs.shape is', targets[1]['labels'].shape, targets[1]['masks'].shape)
                # print('aux_outputs.shape is', targets[2]['labels'].shape, targets[2]['masks'].shape)
...
def train():
    ...
    if hp.mode == '3d':
        ...
        from models.three_d.TransUnet3d import TransUnet3d
        model = TransUnet3d()#input_channels=hp.in_class, out_channels=hp.out_class, init_features=8)#, base_n_filter=2)  #2
    ...
    # print(epochs, args.epochs, elapsed_epochs)
    # quit()
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # print('outputs[pred_masks]', outputs['pred_masks'].shape)
            # print('outputs[aux_outputs]', len(outputs['aux_outputs']))
            # for ii in range(len(outputs['aux_outputs'])):
                # print(ii, outputs['aux_outputs'][ii].keys())
